...
UPDATE EMPLOYEE t
SET t.MANAGER_ID = (SELECT c.MANAGER_ID FROM CSVREAD('~/Documents/projects/mcp-human-resources/employee_data_load.csv') c WHERE CAST(c.EMPLOYEE_ID AS BIGINT)= t.EMPLOYEE_ID)
WHERE t.EMPLOYEE_ID IN (SELECT CAST(c.EMPLOYEE_ID AS BIGINT) FROM CSVREAD('~/Documents/projects/mcp-human-resources/employee_data_load.csv') c);  